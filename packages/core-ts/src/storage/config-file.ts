...
export const DEFAULT_CONFIG_PATH = join(paths.config, 'config.enc')
...
async function getKey(iterations = PBKDF2_ITERATIONS): Promise<CryptoKey> {
...
async function loadStore(): Promise<ConfigStore> {
  ...
  try {
    ...
    try {
      ...
      // Re-throw original error if legacy also fails
      throw err
    }
  }
}
