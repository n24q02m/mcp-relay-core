...
class TestPBKDF2Migration:
    async def test_auto_migrates_legacy_config(self, _temp_config):
        ...
        # Test migration from 600k (LEGACY) to 1M (current)
        ...
        # Legacy key (600k) should no longer decrypt
        with pytest.raises(InvalidTag):
            decrypt_data(legacy_key, new_data)

        # Test migration from 100k (V1_LEGACY) to 1M
        v1_legacy_key = derive_file_key(
            machine_id, username, V1_LEGACY_PBKDF2_ITERATIONS
        )
        encrypted_v1 = encrypt_data(v1_legacy_key, json.dumps(store))
        config_path.write_bytes(encrypted_v1)
        ...
        with pytest.raises(InvalidTag):
            decrypt_data(v1_legacy_key, new_data)
