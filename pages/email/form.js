...
function createAccountCard(idx, fieldsContainer, submitBtn) {
  ...
  if (idx > 0) {
    ...
    removeBtn.addEventListener('click', () => {
      card.remove()
      updateAccountNumbers(fieldsContainer)
    })
...
function updateAccountNumbers(fieldsContainer) {
...
function collectAccounts(fieldsContainer) {
  const cards = fieldsContainer.querySelectorAll('.account-card')
  const accounts = []
...
initForm({
  onFieldsRendered: ({ fieldsContainer, submitBtn }) => {
    // Add first account
    fieldsContainer.appendChild(createAccountCard(accountIndex++, fieldsContainer, submitBtn))
    ...
    addBtn.addEventListener('click', () => {
      fieldsContainer.insertBefore(createAccountCard(accountIndex++, fieldsContainer, submitBtn), addBtn)
    })
    fieldsContainer.appendChild(addBtn)
  },
  collectConfig: ({ fieldsContainer }) => {
    const accounts = collectAccounts(fieldsContainer)
